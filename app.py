...
def init_connection():
    ...
    if "gcp_service_account" in st.secrets:
        try:
            # Ambil nilainya dulu (bisa berupa String atau Object/Dict)
            secret_value = st.secrets["gcp_service_account"]
            
            # --- LOGIKA AUTO-DETECT ---
            if isinstance(secret_value, str):
                # KONDISI A: Format JSON String ("""{...}""")
                # Kita harus pakai json.loads()
                creds_dict = json.loads(secret_value)
            else:
                # KONDISI B: Format Native TOML ([header])
                # Sudah jadi object, tinggal convert ke dict biasa
                creds_dict = dict(secret_value)
            # --------------------------

            # Fix Newline (Wajib untuk kedua kondisi)
            ...
            st.error(f"❌ Error memproses Secrets Cloud: {e}")
            return None

    # 2. FALLBACK KE LOCAL (Jika secrets tidak ada)
    ...
    # 3. FINALISASI
    if creds is None:
        st.error("⚠️ Kredensial tidak ditemukan. Cek Secrets atau file credentials.json.")
        return None

    try:
        gc = gspread.authorize(creds)
        # Ganti "DB_MD" sesuai nama file spreadsheet Anda
        return gc.open("DB_MD")
    except Exception as e:
